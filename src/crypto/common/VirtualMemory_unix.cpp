...
void *rxs::VirtualMemory::allocateOneGbPagesMemory(size_t size)
{
#   ifdef RXS_OS_LINUX
...
void rxs::VirtualMemory::freeLargePagesMemory(void *p, size_t size)
{
    munmap(p, size);
}
...
bool rxs::VirtualMemory::adviseLargePages(void *p, size_t size)
{
#   ifdef RXS_OS_LINUX
